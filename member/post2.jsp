...
rs2.next();
int total = rs2.getInt("total");  // 총 데이터 값 ;
...
///select a1, ( a2||' '||a3||' '||a4||' '||a5||' '||a6||' '||a7||' '||a8) as addr  from post3  where a4 like '%노암%' or a5 like '%노암%';
%>
...
function fn_action() {
		...
		//address = address.replace("  "," "); 치환 
		//alert(address);
		
		// 우편번호, 주소 분리
		//var zipcode = address.substring(0,6); 	// 우편번호,
		//var addr    = address.substring(7); // 주소
		
		var array   = address.split(" "); // 공백 으로 자르기;
		var zipcode = array[0]; 
		var addr    = address.replace(zipcode+" ", ""); // 치환;
		
		// 우편번호 적용
		opener.document.frm.zipcode.value = zipcode; // 값을 넘겨 주는 것
		// 주소 적용
